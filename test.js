 ...
 * @fileoverview Test suite for `mdast-comment-marker`.
 */
...
test('normalize(value, allowApostrophes)', function (t) {
    ...
    t.equal(marker({
        'type': 'paragraph',
        'children': []
    }), null, 'should work without html node');

    t.equal(marker({
        'type': 'html',
        'value': '<div></div>'
    }), null, 'should work without comment');

    t.equal(marker({
        'type': 'html',
        'value': '<!-- -->'
    }), null, 'should work for empty comments');

    t.equal(marker({
        'type': 'html',
        'value': '<!--foo-->this is something else.'
    }), null, 'should work for partial comments');
    ...
    t.equal(marker({
        'type': 'html',
        'value': '<!--foo bar=-->'
    }), null, 'marker stop for invalid parameters (#1)');

    t.equal(marker({
        'type': 'html',
        'value': '<!--foo bar= qux-->'
    }), null, 'marker stop for invalid parameters (#2)');

    t.equal(marker({
        'type': 'html',
        'value': '<!--foo |-->'
    }), null, 'marker stop for invalid parameters (#3)');
